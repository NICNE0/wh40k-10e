...
def create_battlefield_visualization(battlefield: Battlefield,
    ...
    """Create interactive battlefield map with deployment zones, terrain, objectives, and units."""
    ...
    if p1_deployment_zone and p2_deployment_zone:
        ...
        def _get_rectangles(zone):
            ...
            # Expected: zone.bounds["rectangles"]
            if isinstance(b, dict) and isinstance(b.get("rectangles"), list):
                return b["rectangles"]

            # Fallback: zone.bounds["bounds"]["rectangles"] (if extra nesting exists)
        ...
        def draw_zone(zone, line_color, fill_rgba, label):
            ...
            # Triangle/polygon via vertices, with circle fallback
        ...
        # Draw zones
        draw_zone(p1_deployment_zone, "cyan", "rgba(0,255,255,0.1)", "P1 Deployment")
        draw_zone(p2_deployment_zone, "orange", "rgba(255,165,0,0.1)", "P2 Deployment")

        # Draw center cutout circle if either zone defines it (no-man's-land in the middle)
